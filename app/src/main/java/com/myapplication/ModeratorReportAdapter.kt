...
class ModeratorReportAdapter(
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        holder.btnEdit.setOnClickListener {
            Toast.makeText(context, "Fitur koreksi belum dibuat", Toast.LENGTH_SHORT).show()
        }

        // Tombol tandai selesai
        holder.btnMarkComplete.setOnClickListener {
            val catatan = holder.moderatorNote.text.toString()
            val proofUri = imagePickerMap[position]
            report.id?.let { id ->
                updateStatus(id, "selesai", catatan, proofUri)
            }
        }
            ...
            storageRef.putFile(proofUri)
                .addOnSuccessListener {
                    storageRef.downloadUrl.addOnSuccessListener { uri ->
                        ...
                        db.collection("reports").document(reportId).update(data)
                    }
                }
